        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 rtl:right-auto rtl:left-0 top-full mt-2 z-50 min-w-[160px] rounded-xl border border-silver-anchor/10 bg-canvas-elevated/95 backdrop-blur-xl shadow-2xl overflow-hidden"
            >
              {markets.map((m) => (
                <button
                  key={m.id}
                  ...
                  className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors ${
                    marketId === m.id
                      ...
                      : 'text-ink-muted hover:bg-silver-anchor/5 hover:text-white'
                  }`}
                >
                  <span>{m.flag}</span>
                  <span>{m.label}</span>
    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="bg-canvas-elevated/95 backdrop-blur-md border-b border-silver-anchor/5 overflow-hidden fixed top-0 left-0 right-0 z-[70]"
        >
          <div className="mx-auto max-w-7xl px-5 sm:px-8 py-3 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {markets.map((m) => (
                <button
                  key={m.id}
                  onClick={() => select(m.id)}
                  className="inline-flex items-center gap-1.5 rounded-full bg-silver-anchor/5 border border-silver-anchor/10 px-3 sm:px-4 py-1.5 text-xs sm:text-sm font-medium text-ink-muted hover:bg-brand-500/10 hover:border-brand-500/20 hover:text-brand-400 transition-all"
                >
                  <span>{m.flag}</span>
                  <span>{m.label}</span>
                </button>
            ...
            <button
              ...
              className="p-1.5 text-ink-dim hover:text-white transition-colors shrink-0"
              aria-label="Dismiss"
            >
              <X size={16} />
            </button>
