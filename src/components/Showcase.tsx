...
export default function Showcase() {
  ...
  return (
    <section className="relative py-24 sm:py-32 bg-canvas-elevated/50">
          ...
          {c.projects.map((project, i) => (
            <motion.div
              ...
              className={`group relative rounded-2xl overflow-hidden ${project.span} aspect-[16/10] ${
                project.featured
                  ? 'ring-1 ring-brand-500/30 shadow-lg shadow-brand-500/10'
                  : ''
              }`}
            >
              <img
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-canvas-base/90 via-canvas-base/20 to-transparent" />
              <div className="absolute inset-0 bg-canvas-base/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

              {project.featured && (
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-brand-500/10 via-transparent to-transparent pointer-events-none" />
              )}

              <div className="absolute top-4 left-4 rtl:left-auto rtl:right-4 flex items-center gap-2">
                <span className="inline-block rounded-full bg-brand-500/15 backdrop-blur-sm border border-brand-500/20 px-3 py-1 text-xs font-semibold text-brand-400">
                  {project.category}
                </span>
                {project.featured && project.badge && (
                  <span className="inline-block rounded-full bg-brand-500/90 px-3 py-1 text-xs font-bold text-canvas-base shadow-md shadow-brand-500/30">
                    {project.badge}
                  </span>
                )}
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                ...
                {project.caption && (
                  <span className="inline-block mt-2 rounded-full bg-silver-anchor/5 backdrop-blur-sm border border-silver-anchor/10 px-3 py-1 text-[10px] font-medium tracking-wide uppercase text-ink-muted">
                    {project.caption}
                  </span>
                )}
              </div>
